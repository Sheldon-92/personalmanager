...
> **更新时间**: 2025-09-15 20:20
> **盘点范围**: CLI命令、脚本、集成服务、自动化任务
> **最新功能**: 邮件智能分类（2025-09-15）
...
| `show_all_emails.py` | 显示所有邮件详情 | 调试邮件功能 | ✅ 可用 |
| `test_email_analyzer.py` | 测试邮件智能分析 | 调试邮件分类 | ✅ 可用 |
| `sync_inventory_to_briefing.py` | 同步功能清单到简报 | 自动更新简报 | ✅ 可用 |

**脚本总数**: 26个（2025-09-15更新）
...
| 模块 | 文件位置 | 功能描述 | 状态 |
|------|----------|---------|------|
| EmailAnalyzer | `src/pm/integrations/email_analyzer.py` | 邮件智能分类（Actionable/Informational/Discussion/FYI） | ✅ 可用 |
| 邮件类型识别 | 同上 | 基于关键词识别邮件类型和置信度 | ✅ 可用 |
| 行动项提取 | 同上 | 从邮件中自动提取具体行动项 | ✅ 可用 |
| 截止日期识别 | 同上 | 识别邮件中的时间和截止日期 | ✅ 可用 |
